...
int main()
{
    ...
    while (resp == 's' || resp == 's')
    {
        ...
        switch (opcion)
        {
            ...
            case 3:
            {
                int eliminar = 0, posnegocio = 0, repartidor = 0, pos_producto = 0;
                string tipo_orden, estado_orden;
                cout << "Ingrese posicion del cliente: ";
                cin >> eliminar;
                pedidoscliente++;
                cout << "Ingrese posicion del negocio: ";
                cin >> posnegocio;
                cout << "Ingrese posicion de repartidor: ";
                cin >> repartidor;
                ordenesrepartidor++;
                cout << "Ingrese tipo de orden: ";
                cin >> tipo_orden;
                cout << "Ingrese estado de la orden: ";
                cin >> estado_orden;
                cout << "Ingrese producto: ";
                cin >> pos_producto;
                ordenesempleado++;
                Cliente* temp = dynamic_cast<Cliente*>(personas[eliminar]);
                temp->setpedidoshechos(pedidoscliente);
                Repartidor* temprep = dynamic_cast<Repartidor*>(personas[repartidor]);
                temprep->setordenesatendidas(ordenesrepartidor);
                for(int i = 0; i < personas.size(); i++){
                    if(dynamic_cast<Empleado *>(personas[i])){
                        Empleado* temp = dynamic_cast<Empleado* >(personas[i]);
    ...
    for(int i; i < negocios.size(); i++){
        delete negocios[i];
        negocios[i] = NULL;
    }
    for(int i; i < ordenes.size(); i++){
        delete ordenes[i];
        ordenes[i] = NULL;
    }
}
